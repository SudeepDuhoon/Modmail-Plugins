...
import discord
from discord.ext import commands, Config
...
class BrainShop(commands.Cog):
    ...
    async def _view(self, ctx: commands.Context):
        ...
        async with self.config.guild(ctx.guild).all() as guild_settings:
            ...
            settings += f"**Auto-Channels:** {trigger_typing(setting_channels['channels']) or None}\n"
            settings += f"**Allowlist:** {trigger_typing(setting_channels['allowlist']) or None}\n"
            settings += f"**Blocklist:** {trigger_typing(setting_channels['blocklist']) or None}"
